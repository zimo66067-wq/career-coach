...
> **C0 是当前证据快照，不是就业概率，也不是预测。**
>
> 历史版本在此之上定义了 `C7_low` / `C7_high`（用固定的 0.30 / 0.70 假设把 C0
> 外推到「第七天」）。该口径已于 **2026-09-13** 删除：它是预测型展示，而 0.30 / 0.70
> 只是 MVP 演示参数，没有任何统计依据。现在输出到 `AbilityProfile.baseline` 的只有 C0 与
> 六维分，schema 中的 `scenario_day7` 字段一并移除。
>
> 后续的「补齐证据 → 复测」闭环由 F4 重构为 Gap & Action Plan 承担：变化只能靠**重新测量**
> 确认，而不是靠区间外推。
...
**对拍期望值**：R=73.00，M=60.00，I=72.55，C0=68.27（±0.5）。
