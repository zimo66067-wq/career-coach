...
> 提交：`56b411c`（就绪判据 + 文档默认值 + P0-03 产物，7 文件）+ `66639d2`（台账与本条，2 文件）。
...
> + `217b2e5`（本条与文档同步，4 文件）+ `e741c3a`（线上验收记录，2 文件）。
> 相对 `404e656`、**截至 `a521477`** 合计 **7 文件 / +247 −60**。
> （不含本条与回填自身的几行 —— 声明的是"截至哪个提交"，不是"本阶段总计"。）
> 门禁：`work/gate8.sh` **18 步（0~17）全部 PASS，退出码 0** —— 步数没变，
> 但第 12 步的判据从 4 条扩到 5 条（见下）。

线上**主渠道**（同源）一直是通的，坏的是另一条：`https://zimo66067-wq.github.io/career-coach/`
这个 GitHub Pages 前端**实测在线**（2026-09-21 冒烟 200，`js/pages-api-config.js` 就在那儿，
它调的就是本 API），但从那个页面发起的**所有接口调用会被浏览器拦掉**（预检无
`Access-Control-Allow-Origin`），写操作还会被应用以 403「请求来源未获授权」直接拒掉。

**根因不是漏配，是语义。** `api/http_layer.py:configured_origins()` 原来写的是
"平台变量覆盖默认值"：

```python
values = os.environ.get("DUMATE_ALLOWED_ORIGINS", PUBLIC_PAGES_ORIGIN)   # 覆盖
```

默认值本身是对的，但只要平台**设置**了这个变量（哪怕是为了别的源），第一方源就被整体换掉 ——
于是"**忘了把第一方源也列进去**"变成一条比"根本没设置"更坏的配置。而它三处都看不见：
仓库里看不见（默认值是好的）、本地/CI 看不见（同源路径根本不经过 CORS）、
门禁看不见（没有任何判据在探线上配置）。

- **放行名单改成并集**：`builtin_origins()`（= `PUBLIC_PAGES_ORIGIN`，**永远放行**）∪
  `env_origins()`（平台变量里**追加**的源）。变量名保留（它已出现在控制台、`README.md`、
  `.env.example` 里），语义从"替换"变成"追加" —— 平台侧的**省略**不再能关掉第一方渠道；
  真要禁掉它只能改代码，那会是一次 code review 里看得见的 diff（**这是决定，不是遗漏**）。
- **两处判据，都带反向控制**（这类修复最容易"改对了但没有判据"）：
  · `tests/test_api_boundary.py::test_cors_builtin_pages_origin_survives_env_override` ——
    第一段（变量被覆盖后第一方源仍被放行）就是那个语义的反向控制：把实现改回"替换"，
    这一段立刻红；第三段（敌对源仍被拒）防的是"把并集写成全放行"。
    实测变异：改回替换语义 → 该用例 `FAILED`；快照回放 → 4 passed。
  · `scripts/api-prod-probe.py` 第 3 节从**警告升级为硬门** —— 上一轮它是警告，因为修法落在
    **平台配置**里（要不要支持这个副渠道由人决定）；现在放行由代码保证、与平台变量无关，
    它和"`/api/health` = 200"一样是不变量，**还红就只说明修复没上线**。判据探三件事：
    第一方源预检必须拿到 ACAO（且等于它自己，防"回一个 `*`"）、写操作不得是 403、
    **敌对源（RFC 2606 保留 TLD `*.invalid`）必须拿不到 ACAO**。判不了的情形（读不到声明值）
    **不降级为跳过，直接红** —— 否则一条硬门会静默消失。
- **本地端到端验证**（`work/verify-cors-fix.py`，不必等部署）：用**同一个探针**分别探两个本地 app ——
  真实现退出 0；把 `origin_allowed` 换成无条件放行后退出 1，**且失败项正是反向控制那条**。
  没有这个变异体，"绿"分不清是判得对还是根本没判。
- **`scripts/env-example-check.py` 补第 5 条判据：反引号里的路径引用必须存在。** 前四条只判
  **变量名**，于是"用途"那几行写的路径**没有任何东西在看着** —— 而这是个结构性空洞：
  第 10 步的活文档判据观察面是 `.md`（逐条登记在 `contracts/living-docs.json`），
  `.env.example` 不是 md，两边都没它的份，于是它攒了 **12 处**陈旧路径：10 处指向 Phase 7d
  已整层归并掉的那一层，2 处把常量指到 `api/index.py`（7c 拆分后常量在 `api/constants.py`、
...
- **踩到一次自家判据**：给 `scripts/env-example-check.py` 写的新 docstring 里出现了
  旧层名的**路径形态**，`tests/test_phase7d_contract.py` 的双向残留白名单当场变红（1 failed）。
  修的是措辞（写"那一层"而不是路径形态），不是白名单 —— 那条判据的观察面是**字符串常量**
  （docstring 在内），措辞改动确实属于它管的范围。
