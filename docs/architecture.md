...
>
> **Phase 8 更新（2026-09-20）**：§4 的服务层清单是 2026-09-13 的**测量快照**，此后 Phases 3~6
> 又新增了 4 个 service；而表里的 `task_service.py` 已随 F2 后端删除（见上方 Phase 1 更新行）。
> §4 标题据此补上「Phase 0 基线」声明 —— 该节的**数字与清单都是那一次的测量值**，不是现状；
> 现状以 `services/` 目录为准（2026-09-20 实测：10 个文件 / 3159 行）。
> 触发这次修正的是判据扩面：**反引号里的裸 `.py` 名不在观察面内**（本节与
> `tests/TEST_PLAN.md` 都有这种写法）。Phase 8 量了影响面（23 处）之后决定**不扩**
> —— 同一写法混了"现在该存在的文件 / 计划产出的文件名 / 仓库外的脚本"三种语义，
> 收了就是制造假阳性。理由记在 `contracts/living-docs.json` 的 `excluded_forms`。
...
## 4. Service 层（Phase 0 基线：7 个文件 / 1593 行）
