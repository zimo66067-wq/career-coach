...
| 8 | P0-03 端到端真实数据闭环 | **【我】** | ⚠️ **判据已补全（2026-09-23），但还缺一次带密钥的端到端复跑。** 原缺陷：`resume_report` 与 `jd_match_explain` 被喂**字面占位符**（`同上简历文本` / `简历同上，JD同上`），模型如实拒答，而脚本**只判 HTTP 层 `status`** ⇒ 拒答被计成 success（"21/21 成功"的成因）。现已修三处：① 用 `tests/fixtures-synthetic/` 的**真实样本**构造结构化输入（`resume_report` 2418 字符、`jd_match_explain` 5346 字符；四态结果由规则引擎 `services/match_service.py::match_job_profile()` 算出），**不再回退占位符**（构造失败就记 `skipped`，不产假绿）；② 新增**内容断言** `judge_output()` —— 空输出 / 命中拒答标记 / 长文过短 一律记 `INVALID`、**不计入 success**，标记全部取自实测拒答原文；③ 统计口径新增 `invalid` / `skipped`，退出码改为「有失败**或**有无效即非 0」。**离线反向控制已过**（`work/verify-p0-03-content-assertion.py`：4 个真实拒答样例全判红、2 个正常长文判绿）。**仍缺**：无密钥做端到端复跑，"拒答已消除"尚无实测证据 —— 拿到 key 后跑一次即可闭环。另注：这两个任务类型在本仓库**没有调用方**（属 DuMate 侧），保留还是移出列表**属产品口径，未擅自改动**。**不阻断上线** —— 真实路径（`resume_diagnosis` / `jd_extract`）已通过。 |
