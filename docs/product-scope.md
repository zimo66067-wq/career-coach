...
#### 2026-09-23 补充裁决：大陆优先、官方开放数据试点

产品负责人确认：**覆盖目标以中国大陆为先；第一批只试官方开放数据，不采购商业招聘 API。**这决定了来源筛选方向，**不等于数据已接入或已达到实时性**。首个验证区域定为北京，其他省市待逐个核实来源、字段、授权及更新情况后扩展；不把北京样本称为“全国单位库”。

首选候选来源是[北京市公共数据开放平台「单位招聘岗位信息」](https://data.beijing.gov.cn/zyml/ajg/srlsbj/466e22a6b0314b4298864dcfb1a50803.htm)，发布方为北京市公共数据开放平台、提供方为北京市人力资源和社会保障局。目录显示“无条件开放”、计划更新频率 15 天、20220 条记录，但**页面可核实的最后更新日期为 2026-07-02**（本次核对日 2026-09-23），不能把目录频率当成当前数据新鲜度。2025 年[开放计划](https://data.beijing.gov.cn/docs/2025.pdf)列有单位名称、岗位名称、要求、地点和登记起止时间；实际可取得字段仍须以下载后的数据和字典为准。

平台[法律声明](https://data.beijing.gov.cn/zyml/ajg/srlsbj/466e22a6b0314b4298864dcfb1a50803.htm)称政务数据可免费、非排他使用与传播，同时要求注明“北京市公共数据开放平台”来源，并及时备案应用情况；平台保留对公开应用进行审核的权利。数据[调用说明](https://data.beijing.gov.cn/cms/web/bjdata/api/dataDoc.jsp?contentID=11087)要求登录后的个人唯一标识码。**目前尚未取得本项目的调用标识码、下载样本、应用备案或审核结果**，故不能将候选来源登记为“已接入”。国家企业信用信息公示系统不能因为公开可查就当作批量数据 API；其[管理办法](https://www.samr.gov.cn/xyjgs/flfg/art/2023/art_f6972d9dbbf641168e71fab9de31cb7a.html)对数据开放设有审批流程。

首批产品口径：仅在真实数据可取得且有源头/日期时展示**北京试点范围的单位名称与岗位历史记录**；名称来自招聘记录不代表主体登记状态已核验。只有源头提供有效期、最近核验时间满足 24 小时门槛，并经原发布页或接口确认仍在招时，才可标“当前招聘”；未达门槛时不得标“实时职位”，继续提供用户手填 JD 的路径。岗位中的联系人和电话不作为搜索索引或对外展示字段。外部付费数据预算暂定 0；开放数据平台账号、实际 API 配额和系统运行成本仍待实测，不能虚构 SLA 或全国覆盖。

D3 状态由“尚未选择方向”改为“**方向与首个候选来源已定，接入前置条件未齐**”。原 2026-10-13 复核期限保留：在此之前取得可用数据、核实字段和授权义务、验证更新频率与成本、明确纠错责任；若不满足原表的五项条件，继续保持封存，不对外开放搜索入口。
...
Phase 7b（`/api/wf03/*` 后端路由去留、`public/*.md` 公开范围）—— **已完成，见 §20**；
Phase 7c（`api/index.py` 拆分，实测 1621 行 → 24 模块）—— **已完成，见 §21**；
7d（`tools/` → `domain/` + `providers/` 归并）—— 未开始；
D3 期限 **2026-10-13**。
...
**下一步**：Phase 7a（门禁扩面与文档真相）—— **已完成，见 §19**；
Phase 7b（wf03 路由去留 + 公开文档范围）—— **已完成，见 §20**；
Phase 7c（拆 `api/index.py`）—— **已完成，见 §21**；7d 见 §21.6。
D3 期限 **2026-10-13**。
...
**下一步**：Phase 7b —— **已完成，见 §20**；7c 见 §21；7d 见 §21.6。
`D3`（单位/职位检索）期限 **2026-10-13** 不变。

---

## 20. Phase 7b 完成记录（2026-09-17 · wf03 路由去留 + 公开文档范围）

Phase 7 的第二个子阶段。**没有新能力，对外行为零变化**。完整报告见 `docs/phase7b-report.md`。

**口径一句话**：两件事都**先问对问题再动手** —— 「消费者是谁」与「这个集合现在是什么」。
两处此前各被挂起过一次（6a / 6b-1），**挂起不是决定，而且挂起没有触发条件**，
所以都在本轮第三次出现。

### 20.1 `/api/wf03/*` → 保留（不是死接口）

上一轮的推理链断在最后一步：**「前端没人调」被读成了「没人调」。**
实测消费者全在浏览器之外：

| 消费者 | 证据 |
| --- | --- |
| DuMate 工作流层 | `wf01`~`wf07` 是产品自己的分类法；`GET /api/health` 的 `workflows` 段报告 `"wf03": "available"` |
| 公开能力声明 | `capability_matrix.md` 的 **N7 / N9** 把 WF-03 标为**已验证**；`dumate-workflow-sop.md` 有 WF-03 搭建节 |
| 彩排链 | `scripts/run-rehearsal.py`：consent → diagnose → **jd → match** → interview → ability → delete |

**与 7a 那条教训同形**：判据的观察面（前端）窄于它要判的语义（有没有消费者）。
7a 表现为判错文案，7b 表现为**差点删掉一份公开能力声明** —— 后者代价更大。
判据：`tests/test_phase7b_contract.js` 7b-1（四条链都在）/ 7b-2（证据链在位 + 反面断言
`data-bridge.js` 仍不写死 wf03）。

### 20.2 `public/*.md` → 16 份公开，且这个集合现在是可判的

Vercel 静态根是 `public/`，所以**把文档放进 `public/` 与把它发布出去是同一个动作**。
此前没有人能回答"当前公开集合是什么"：`public/index.md` 只索引 12 份、
`dependency-map.md §4.5` 只列 7 份、镜像判据明确排除 `.md`。
**6a 与 6b-1 写「属产品/隐私决策」不是推诿 —— 当时确实没有可判的对象。**

实测 **16 份**（15 顶层 + `blind-test-results/blind-test-report.md`），
`sensitive-scan.py` 全部无发现，**无泄露**：它们是赛事提交物与过程证据，公开是有意的。
所以本轮**一份都没撤**，而是把结论变成对象：

| 物 | 角色 |
| --- | --- |
| `contracts/publish-scope.json` | 逐份记录路径 + 用途分类 + **公开的理由**；改它 = 一次有记录的公开范围变更 |
| `scripts/publish-scope-check.py` | 全向比对（增 / 删都红）+ 空判防线 + 8 项自检探针 |
| `tests/test_phase7b_contract.js` 7b-4/7b-5 | 清单结构 + **独立复算**集合一致性（两条实现互不调用） |

撤出公网现在是一步的事：删文件 + 改清单（`.md` 不在镜像不变量内，不影响两棵树的非 md 一致性）。
**要不要撤是产品决定；7b 只负责让"当前是什么"可判。**

### 20.3 顺带修掉的真缺陷

- **`public/index.md` 是别的树的索引**：标题写成「docs 文档目录索引」，内容是 `docs/index.md`
  的真子集，且**漏掉自己树里 4 份 md**。与 6a 修掉的「死树自述」同类 ——
  自述文件描述的不是它所在的那棵树。现改名「职跃AI · 公开文档索引」，列全本树 15 份。
- **两棵树的 `README.md` 恢复一致**：6a 刻意对齐过，`.md` 不在镜像不变量内，
  所以这条一致性此前没有判据。现由 7b-3 守住。

### 20.4 判据抓出了正文的事实错误

7b-2 第一次红，**错的是判据**：我断言「WF-03 的证据行指向 `tests/test_api.py`」，
而实测那两行的证据列是 `[CAP-008]` / `[CAP-010]`（`tests/test_api.py` 是 WF-02/WF-04 的写法）。
我把别处看到的模式顺手填进了证据列 —— **判据除了防回归，还能抓出正文的事实错误**，
这是本轮新认识到的第二种价值。

### 20.5 门禁

**12 步 → 13 步**，全绿：482 pytest / **85 node**（77 → 85）/ 32 schema / 252 文件敏感扫描 /
**dead = 0** / DDL 15 / 冒烟 70-70 / 镜像 30 文件逐字节同 / 活文档全解析 / 22 前端写死路径 /
29 env 变量 / **公开范围 16 份与清单一致**。
变异注入 **12/12 全部被抓到**。

### 20.6 遗留（Phase 7 还剩两段）

| 段 | 内容 | 规模（实测） |
| --- | --- | --- |
| **7c** | `api/index.py` 拆分 | **1621 行** |
| **7d** | `tools/` → `domain/` + `providers/` 归并 | **6252 行** / 20+ 模块；`dependency-map.md` 的保留期**已到期** |

**下一步**：Phase 7c（拆 `api/index.py`）—— **已完成，见 §21**；7d 见 §21.6。
`D3`（单位/职位检索）期限 **2026-10-13** 不变。

---

## 21. Phase 7c 完成记录（2026-09-17 · 拆 `api/index.py`）

Phase 7 的第三个子阶段。**纯结构重构，对外行为零变化** —— 逻辑一行没改写。完整报告见
`docs/phase7c-report.md`。

**口径一句话**：7c 的风险**不在业务逻辑**（那是逐行搬的，且可证），而在**结构塌陷** ——
"拆了个寂寞"、少 import、依赖回头成环、再导出面漂移。所以本轮的产出是**结构判据**，
不是新功能。

### 21.1 结果

**1621 行的入口 → 24 个模块，入口只剩 108 行。**

| 量 | 值 |
| --- | --- |
| `api/index.py`（拆分前 → 后） | 1621 行 → **108 行** |
| 其中 `route_api()` | 917 行（原文件 664–1580），含 38 个 `if route ==` + 12 个 `route.startswith(` |
| 被搬走的区间 | 688–1578 共 891 行，**非空 831 行**（去重 598） |
| 模块划分 | `api/` 10 个基础设施模块 + `api/handlers/*` 14 个业务族 |
| 入口本地规则表 | 49 条 = 34 非参数化（`/api` + 33）+ 15 参数化 |

**"逐字迁移"是可核验的，不是一句话**：把原文件分支区间与 14 个 handler 的函数体各自做
"非空行 → 计数器"，两边**逐行相等含重数**（831 / 831）。比对方（基线副本）也与
`git show 131f83d:api/index.py` **逐字节相同** —— 否则比的是一个来历不明的文件。

### 21.2 三件必须留在入口的东西

| 留什么 | 为什么搬了会坏 |
| --- | --- |
| 文件名 `api/index.py` + 变量名 `app` | `vercel.json` 的 `functions` 指向它；`vercel-dead-routes.py` 是 `from api.index import app`。**部署契约，不是实现细节** |
| `REPOSITORY_ROOT` + `sys.path` 插入 | 线上入口就是本文件，`tools/` / `services/` 能被 import 全靠这三行 |
| 本地测试路由表 | 本地 `test_client()` 与线上 `?_route=` 重写的**对照面**，`vercel-dead-routes.py` 靠它证明"本地绿 == 线上可达" |

### 21.3 唯一的新失效模式，与它专属的判据

拆 871 行到 14 个模块，**唯一的新失效模式是"某个模块少 import 一个名字"** ——
它只在**没被用例覆盖的那条分支**上 `NameError`，于是 **pytest 会绿、评审会漏**。
前 13 步门禁里没有任何一步看得见它（第 1 步只在覆盖到时报错；第 5 步只看路由名在不在；
第 9~13 步看的是前端与文档）。

所以新增**第 14 步 `scripts/api-import-check.py`**：用 `symtable`（**解释器自己的作用域规则**）
判每个 `api/` 模块里有没有"全局读过、却没在模块级绑定"的名字。
不用 `compile()`（只查语法，不查名字），也不自己拿 `ast` 重写一遍作用域规则（那是重实现，容易错）。
含 **13 项自检探针**，其中 **3 项是"必须报错"的反向探针**，第 3 项正是本轮的形态：
「只在某条分支上用的漏 import 也必须报出来」。

### 21.4 门禁自己的两个洞（本轮最值得记的一条）

**洞一：`git diff --check HEAD` 那一格是空判。** 脚本把 PATH 收窄成 `/usr/bin:/bin:...`，
而 PortableGit 的 `git.exe` 在 `mingw64/bin`。于是 `git` 不在 PATH 时那格执行的是
`command not found`(127)，而写法是 `git diff --check HEAD && echo "clean"` ——
`&&` 短路，**那格什么都不打印，读起来和 `clean` 一模一样**。
实测：`gate7b.log` 的那一格就是空的（**应视为未验证**），6b-2/6b-3/7a 那三次才是真跑了。

**洞二：门禁恒以 0 退出。** 所以"看退出码"这件事本身是空判 —— 本轮第一次后台跑门禁时，
外层报 `GATE_EXIT=127`，而**门禁其实一步都没跑**。

> **可复用的一条**：判据的失效有两种 —— **"判错"与"没被执行"**。本项目此前防的一直是第一种
> （观察面比语义宽 / 窄，四次），**7c 撞到的是第二种，而它对"全绿外观"的贡献完全一样**。

### 21.5 判据抓出正文错误（第三次），以及"会自我漂移的数字"

新 docstring 引用的规模数字**第一稿错了四处**：入口「101 行」实测 105、
「39 条 `if route ==`」实测 38、「50 条本地路由规则」实测 49、「参数化 13 条」实测 15
（另有「919 行 `route_api()`」实测 917）。`tests/test_migrations.py` 的一句注释也断言
「`api.index` 仍然再导出 `migration_status`」—— **它已经不导出了**。

修法不只是改数字：入口行数写 101 → 实测 105 → **把数字改对的动作又把它变成 106**。
**会自我漂移的量不属于 docstring** —— 这类量改为判**不变量**（`api/` 相对原文件的膨胀比
1.457，落在 1.3~1.6），只记快照。新增 `work/verify-numbers.py` 把每条断言与它的算法钉在一起。

### 21.6 遗留（Phase 7 只剩一段）

| 段 | 内容 | 规模（实测） |
| --- | --- | --- |
| **7d** | `tools/` → `domain/` + `providers/` 归并 | **6252 行** / 20+ 模块；全部 import 需重写。`dependency-map.md` 的保留期**已到期**，不构成"可再推一轮"的选项 |

**本轮明确不动的**：`TRACE_ID_PATTERN` 死常量照搬保留（7c 的 DoD 是"拆分"不是"删代码"，
混进来会让"对外行为零变化"这句话变浑）；`api/constants.py` 里那个"哑绑定"只保留生效的一份
